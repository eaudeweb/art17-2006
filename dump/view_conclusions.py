# Script (Python)
# /article17/speciessummary/view_conclusions
...
if auth_roles['administrator'] or auth_roles['expert']:
    return [(concl['region'], concl['assesment_speciesname'], concl['user'], concl['MS']) for concl in conclusions]
...
#is anonymous user or has role 'stakeholder' or 'nat'
    ...
    if ok_concl:
        buffer = []
        buffer = [(concl['region'], concl['assesment_speciesname'], concl['user'], concl['MS']) for concl in ok_concl]
        ...
        for rec in conclusions:
            ...
            if not user_roles['administrator'] and not user_roles['expert']:
                buffer.append((rec['region'], rec['assesment_speciesname'], rec['user'], rec['MS']))
        return buffer
    #if no decision with OK or END is found then display the 'automatic' conclusions
    else:
        buffer = []
        buffer = [(concl['region'], concl['assesment_speciesname'], concl['user'], concl['MS']) for concl in conclusions if concl['user'] in ['maximiur', 'iurieetcbd']]
        #check if the record was inserted by a STA
        for rec in conclusions:
            user_roles = context.acl_manager.getSpecificUserRoles(rec['user'])
            if not user_roles['administrator'] and not user_roles['expert']:
