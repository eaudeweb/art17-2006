# Script (Python)
# /article17/speciessummary/attrs_content/td_habitat_surface_area
...
for err in qa_errors:
    if err['FlagField'] in ['conclusion_habitat', 'percentage_habitat_surface_area']:
        title.append('%s: %s' % (err['FlagText'], err['suspect_value']))
        class_names.append('qa_error')


output.update({
    'content':context.str2num(record['percentage_habitat_surface_area']),
    'title': '<br />'.join(title),
