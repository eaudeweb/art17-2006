# Script (Python)
# /article17/habitatsummary/insertConclusion
# params: ''
...
if valid_data and enable_warning:
    ...
    records = context.sql_methods.lookup_method_conclusions(habitatcode=form.get('habitatcode', ''), region=form.get('region', ''))
...
if valid_data:
    ...
    context.sql_methods.insert_habitat_manual_assesment(MS='EU25', region=request.form.get('region', ''), habitatcode=request.form.get('habitatcode', ''),
                            ...
                            user=request.AUTHENTICATED_USER, last_update=DateTime().strftime('%Y-%m-%d %H:%M'), 
