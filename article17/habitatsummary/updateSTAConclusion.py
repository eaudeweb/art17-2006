# Script (Python)
# /article17/habitatsummary/updateSTAConclusion
# params: ''
...
if ms == 'EU25':
    if valid_data and enable_warning:
        ok_conclusions = context.sql_methods.lookup_OK_manual_assessment(habitatcode=habitat, region=region)
        errors = []
    ...
    if valid_data and enable_warning:
        assessment = context.sql_methods.lookup_country_assessment(habitatcode=habitat, region=region, eu_country_code=ms)
        errors = []
...
if valid_data:
    ...
    context.sql_methods.update_user_habitat_manual_assesment(MS=ms, region=region,
