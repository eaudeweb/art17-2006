# Script (Python)
# /article17/speciessummary/updateConclusion
# params: ''
...
if valid_data and enable_warning:
    ...
    records = context.sql_methods.lookup_method_conclusions(assesment_speciesname=context.string_decode(assesment_speciesname), region=region)
...
if valid_data:
    ...
    if decision == 'OK':
        ...
        decision = ''
    context.sql_methods.update_user_manual_assesment(MS='EU25', region=region, 
                        assesment_speciesname=context.string_decode(assesment_speciesname), 
                        range_surface_area=range_surface_area, method_range=method_range, range_trend=range_trend, range_yearly_magnitude=range_yearly_magnitude, complementary_favourable_range=complementary_favourable_range, 
                        population_size=population_size, population_size_unit=population_size_unit, method_population=method_population, population_trend=population_trend, population_yearly_magnitude=population_yearly_magnitude, complementary_favourable_population=complementary_favourable_population, 
                        habitat_surface_area=habitat_surface_area, method_habitat=method_habitat, habitat_trend=habitat_trend, complementary_suitable_habitat=complementary_suitable_habitat, 
                        conclusion_range=conclusion_range, conclusion_population=conclusion_population, conclusion_habitat=conclusion_habitat, conclusion_future=conclusion_future, 
                        method_future=method_future, conclusion_assessment=conclusion_assessment, method_assessment=method_assessment, 
                        user=request.AUTHENTICATED_USER, last_update=DateTime().strftime('%Y-%m-%d %H:%M'), decision=decision)
