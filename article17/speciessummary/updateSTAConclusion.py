# Script (Python)
# /article17/speciessummary/updateSTAConclusion
# params: ''
## Script (Python) "updateSTAConclusion"
...
#hidden fields
ms = form.get('ms', '')
species = form.get('assesment_speciesname', '')
...
if valid_data:
    ...
    context.sql_methods.update_user_manual_assesment(MS=ms, region=region, assesment_speciesname=assesment_speciesname, 
                        ...
                        user=auth_user, last_update=DateTime().strftime('%Y-%m-%d %H:%M'), decision=decision)
    return RESPONSE.redirect('%s?group=%s&species=%s&region=%s' % (context.absolute_url(), group, species, region))
else:
    #put data on session
