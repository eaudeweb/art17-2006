# Script (Python)
# /article17/speciessummary/details/config_xml
...
auth_roles  = context.acl_manager.getSpecificUserRoles(auth_user)
...
species_name = context.string_decode(species)
results = context.select_species_countries(region=region[1:-1].replace('^', '').split('|'), assesment_speciesname=species_name)
assesment_speciesname=url_quote(species_name)
...
restricted_species = context.sql_methods.get_map_restricted_species(assesment_speciesname=species_name)
if (species_name.lower() not in context.convertCursorToList(restricted_species, 'assesment_speciesname')) or auth_roles['expert'] or auth_roles['administrator']:
    #show protected species
    layer_OG10 = """<fmc:LayerOGWMS id="OG10" format="PNG" url="art17ws?VERSION=1.1.1&SERVICE=WMS&SRS=EPSG:3035&EXCEPTIONS=INIMAGE&spc='%s'&method=art17wsGridDistS" QUERY_LAYERS="" LAYERS="art_17_Map_Distribution"/>""" % assesment_speciesname
    layer_OG11 = """<fmc:LayerOGWMS id="OG11" format="PNG" visible="false" url="art17ws?VERSION=1.1.1&SERVICE=WMS&SRS=EPSG:3035&EXCEPTIONS=INIMAGE&spc='%s'&method=art17wsGridRangeS" QUERY_LAYERS="" LAYERS="Art_17_Map_Range"/>""" % assesment_speciesname
else:
    #hide protected species
    layer_OG10 = """<fmc:LayerOGWMS id="OG10" format="PNG" url="art17ws?VERSION=1.1.1&SERVICE=WMS&SRS=EPSG:3035&EXCEPTIONS=INIMAGE&spc='%s'&method=art17wsGridDistSR" QUERY_LAYERS="" LAYERS="art_17_Map-Distribution_Sensitivespecies_Removed"/>""" % assesment_speciesname
    layer_OG11 = """<fmc:LayerOGWMS id="OG11" format="PNG" visible="false" url="art17ws?VERSION=1.1.1&SERVICE=WMS&SRS=EPSG:3035&EXCEPTIONS=INIMAGE&spc='%s'&method=art17wsGridRangeSR" QUERY_LAYERS="" LAYERS="Art_17_Map_Range_SensitiveSpecies_Removed"/>""" % assesment_speciesname
...
<FLAMINGO xmlns:fmc="fmc" lang="en" tooltipdelay="500" maxwidth="680" maxheight="400" borderwidth="0">
    ...
    <fmc:Legend left="right -140" top="120" bottom="bottom -30" right="right -2" listento="map1">
      <group label="Assess. layers" open="true">
        ...
        <item canhide="true" infourl="" label="Art17 Diss. Range" listento="OG11"/>
        <item canhide="true" infourl="" label="Art17 Diss. Dist." listento="OG10"/>
      </group>
    ...
    <fmc:Container borderwidth="0" bordercolor="#b8b8b8" left="0" top="0" bottom="bottom -0" right="right -150">
      ...
      <fmc:Map id="map1" movequality="HIGH" left="0" top="0" bottom="bottom" right="right" visible="true" extenthistory="4" extent="%s" fullextent="371363.8956955,-375446.023944,8078238.5599865,7331428.640347">
        ...
        <fmc:LayerOGWMS id="OG13" format="PNG" url="art17ws?method=art17wsLineWork" VERSION="1.1.1" SERVICE="WMS" SRS="EPSG:3035" EXCEPTIONS="INIMAGE" QUERY_LAYERS="" LAYERS="Art_17_linework"/>
        %s 
        %s
